...
useEffect(() => {
  ...
  const transferInfo = `
            ...
            <p><strong>Monto Total: </strong>${(descuento).toLocaleString('es-AR', {
              style: 'currency',
              currency: 'ARS',
            })}</p>
...
// 1. Obtener datos de facturación
const obtenerDatosFacturacion = async () => {
  ...
  return { tipoFactura, datosFactura };
};

// 2. Construir objeto de usuario completo
const construirUserCompleto = (user, datosFactura, tipoFactura, nuevaDireccion) => {
  return {
    ...user,
    factura: {
      ...datosFactura,
    ...
    nombreCompleto: user.nombreCompleto || user.displayName || datosFactura.razonSocial,
    dniOCuit: user.dniOCuit || datosFactura.cuit || datosFactura.dniOCuit || user.documento || '',
    telefono: user.telefono || nuevaDireccion.telefono || '',
...
// 3. Calcular precios
const calcularPrecios = (totalRef, descuento) => {
  const rawSubtotal = totalRef.current?.textContent
    ?.replace('Subtotal: ', '')
    .replace(/\./g, '')
    .replace(',', '.')
    .replace('$', '')
  ...
  const subtotalNumber = parseFloat(rawSubtotal) || 0;
  
  return {
    transferenciaPrecio: subtotalNumber * (1 - (descuento / 100)),
    mercadoPagoPrecio: subtotalNumber
  };
};

// 4. Seleccionar método de pago
const seleccionarMetodoPago = async (transferenciaPrecio, mercadoPagoPrecio, descuento) => {
  ...
  const result = await Swal.fire({
    ...
    html: `
      <div class="payment-method-header">
        <span class="transfer-label">Transferencia (${descuento}% OFF)</span>
        <span>MercadoPago</span>
      </div>
      <div class="payment-method-prices">
        <span class="transfer-price">${formatCurrency(transferenciaPrecio)}</span>
        <span>${formatCurrency(mercadoPagoPrecio)}</span>
      </div>
    `,
  });

  return result.isConfirmed ? 'mercadoPago' : 'transferencia';
};

// 5. Procesar pago con MercadoPago
const procesarPagoMercadoPago = async (cart, userCompleto) => {
...
// 6. Procesar pago con Transferencia
const procesarPagoTransferencia = async (cart, userCompleto, descuento) => {
  const guardarPedidoData = await handleGuardarPedido(userCompleto, cart, descuento);
  ...
  const subirAhora = await Swal.fire({
  ...
  if (subirAhora.isConfirmed) {
    await subirComprobante(guardarPedidoData.orderId);
  } else {
    await Swal.fire({
...
const subirComprobante = async (pedidoId) => {
  ...
  if (formValues?.success) {
    ...
    // Notificación con ID del pedido
    await notificador({ _id: pedidoId });
    setCart([]);
  }
};
