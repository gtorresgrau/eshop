...
export default function UpdateProduct({ isOpenModal, toggleModal, product, categoria, marca }) {
  ...
  const submitUpdateProduct = async (e) => {
    ...
    Object.keys(filteredProducto).forEach((k) => formData.append(k, filteredProducto[k]));

    try {
      Swal.fire({ title: 'Guardando Cambios...', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); } });
      const res = await fetch('api/updateProduct', { method: 'PUT', body: formData });
      const data = await res.json();
      Swal.fire({ icon: 'success', title: 'Cambios guardados', showConfirmButton: false, timer: 1500 });
      removeFromLocalStorage('productos');
      toggleModal();
    } catch (err) {
      Swal.fire({ icon: 'error', title: 'Error al guardar cambios', text: 'Por favor, inténtelo de nuevo más tarde.' });
      toggleModal();
    }
  };
  ...
  return (
    <div>
      <div id="updateProductModal" tabIndex="-1" aria-label="editar producto" className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-10 overflow-hidden ${isOpenModal ? '' : 'hidden'}`}>
        <div className="rounded-none max-w-3xl w-full max-h-full overflow-y-auto">
          <div className="relative p-4 bg-white shadow-sm">
            <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5">
              ...
              <button type="button" onClick={handleToggleModal} className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="updateProductModal" aria-label="editar producto">
                <svg aria-label="boton para editar" className="w-5 h-5" width={20} height={20} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
              </button>
            </div>

            <ProductForm producto={producto} onChange={handleChangeInput} imagenes={imagenes} updateImages={handleUpdateImages} removeImage={handleRemoveImage} onSubmit={submitUpdateProduct} submitLabel="Guardar cambios" marcas={marcas} categorias={categorias} onAddMarca={handleAgregarNuevaMarca} onAddCategoria={handleAgregarNuevaCategoria} onClose={handleToggleModal} />
          </div>
        </div>
      </div>
    </div>
